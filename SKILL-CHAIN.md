...
| Onboarding + activation libs | `src/lib/onboarding.ts` `src/lib/onboarding-progress.ts` `src/lib/onboarding-summary.ts` `src/lib/sync-onboarding.ts` `src/lib/setup-status.ts` `src/lib/install-platform.ts` `src/lib/local-watchlist.ts` `src/lib/local-portfolio.ts` `src/lib/activation/` | onboarding-funnel |
...
| Component estate + shared UI libs | `src/components/` `src/lib/brand.ts` `src/lib/calculators.ts` `src/lib/chart-pack.ts` `src/lib/command-layout.ts` `src/lib/comparison.ts` `src/lib/education.ts` `src/lib/focus-trap.ts` `src/lib/format.ts` `src/lib/gradient.ts` `src/lib/nav-prefs.ts` `src/lib/research-queue.ts` `src/lib/surfaces.ts` `src/lib/ticker-logos.ts` `src/lib/ui.ts` `src/lib/usage.ts` `src/lib/usage-store.ts` `src/lib/native/` `public/` | ux-surface |
