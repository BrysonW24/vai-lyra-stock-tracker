      ...
      <div className="space-y-3 pb-28 xl:pb-6">
        ...
        <section className="terminal-panel rounded-panel p-3">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <h2 className="text-[9px] font-semibold uppercase tracking-[0.12em] text-accent">
              Capital is moving here
            </h2>
            <p className="text-[10px] text-ink-dim">Newest disclosed commitments across tracked themes</p>
          </div>

          <div className="mt-1.5">
            {newestEvents.map((e) => (
              <div key={e.id} className="flex items-center gap-2 border-b border-line py-1.5 last:border-b-0">
                <span className="shrink-0 font-mono text-[9px] text-ink-3">{e.date}</span>
                <p className="min-w-0 flex-1 truncate text-[11px] text-ink-title">
                  {e.actor}
                  {e.beneficiary ? ` -> ${e.beneficiary}` : ''}
                </p>
                {typeof e.amountUsdM === 'number' && (
                  <span className="shrink-0 font-mono text-[10px] font-semibold text-positive">
                    ${e.amountUsdM.toLocaleString('en-US', { maximumFractionDigits: 0 })}m
                  </span>
                )}
                <span className="max-w-[96px] shrink-0 truncate rounded border border-line-strong bg-panel px-1.5 py-0.5 font-mono text-[9px] text-ink-2">
                  {themeNameBySlug[e.theme] ?? e.theme}
                </span>
              </div>
            ))}
            {newestEvents.length === 0 && (
              <p className="py-1.5 text-[11px] text-ink-3">No capital events tracked yet.</p>
            )}
          </div>
        </section>
