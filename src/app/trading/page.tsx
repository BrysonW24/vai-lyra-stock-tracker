      ...
      <div className="space-y-3 pb-28 xl:pb-6">
        ...
        <section className="terminal-panel rounded-panel p-3">
          <div className="flex items-center gap-2">
            <FlaskConical size={14} className="text-pending" />
            <h2 className="text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-title">How we earn the right to go live</h2>
          </div>
          <p className="mt-2 text-[11px] leading-relaxed text-ink-2">
            Live execution stays gated until a strategy <span className="text-ink-title">proves itself on paper first</span>. We test the entire pipeline on the <span className="text-ink-title">Paper Bot</span>: the AI explains readiness, deterministic code builds the order, the same risk engine above gates it, you approve, and fills are simulated at real prices with real fees and slippage. Every simulated trade builds the track record - and clears the readiness gates above - that a live bot would have to pass before a single real order is ever placed. Nothing here is optimised away when execution eventually ships; it is added <span className="text-ink-title">behind</span> this gate, not instead of it.
          </p>
          <a
            href="/paper-bot"
            className="mt-3 inline-flex min-h-[44px] items-center gap-1.5 rounded-cell border border-pending/40 bg-blue-tint px-3 py-1.5 text-xs font-semibold text-pending transition hover:bg-blue-deep/30 sm:min-h-0"
          >
