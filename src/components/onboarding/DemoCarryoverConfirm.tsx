  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-ground text-ink">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -left-24 -top-24 h-80 w-80 rounded-full bg-blue/10 blur-3xl" />
        <div className="absolute right-[-8rem] top-32 h-96 w-96 rounded-full bg-blue-info/15 blur-3xl" />
        <div className="absolute inset-0 bg-[radial-gradient(var(--lyra-line)_1px,transparent_1px)] opacity-50 [background-size:22px_22px]" />
      </div>
      ...
      >
        <div className="w-full">
          <div className="mb-5 flex items-center gap-2.5">
            <BrandLogo size={34} />
            <span className="text-[17px] font-semibold tracking-tight text-ink">{BRAND_NAME}</span>
          </div>

          <h1 className="text-3xl font-semibold leading-[1.1] tracking-tight md:text-4xl">
            Welcome back.<br />
            <span className="bg-gradient-to-r from-blue to-blue-info bg-clip-text text-transparent">
          ...
          <div className="terminal-panel relative mt-6 overflow-hidden rounded-panel border border-line-strong p-4">
            ...
            <ul className="relative space-y-3">
              {rows.map((row, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-panel bg-gradient-to-br from-blue to-blue-info text-white shadow-[0_8px_18px_-6px_rgba(30,99,255,0.65)] ring-1 ring-ink/20">
                    <row.icon size={16} strokeWidth={2} />
                  </span>
                  <p className="text-sm leading-snug">
                    <span className="font-semibold text-ink">{row.label}</span>
                    <span className="text-ink-2"> - {row.sub}</span>
                  </p>
                </li>
              ))}
            </ul>
          ...
          {error && (
            <p className="mt-4 rounded-cell border border-negative/40 bg-negative/10 px-3 py-2 text-xs leading-relaxed text-negative-soft">
              {error}
            </p>
          )}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              ...
              onClick={onSave}
              disabled={saving}
              className="inline-flex min-h-[44px] items-center gap-2 rounded-cell bg-[image:var(--lyra-cta-gradient)] px-5 py-3 text-sm font-semibold uppercase tracking-[0.12em] text-white shadow-[0_12px_30px_-10px_rgba(67,209,139,0.6)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              onClick={onReview}
              disabled={saving}
              className="inline-flex min-h-[44px] items-center gap-2 rounded-cell border border-line-strong bg-panel px-5 py-3 text-sm font-medium text-ink-2 transition hover:border-line-hair hover:text-ink disabled:opacity-60"
            >
              Review &amp; edit
            </button>
          </div>
