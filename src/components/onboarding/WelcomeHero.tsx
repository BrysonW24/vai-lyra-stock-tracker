...
export function WelcomeHero({ onChoosePath }: WelcomeHeroProps) {
  const [selected, setSelected] = useState<PathId>('full_setup');

  const paths: { id: PathId; title: string; description: string; icon: LucideIcon }[] = [
    ...
    { id: 'full_setup', title: 'Full setup', description: 'Watchlist, portfolio, goals and alerts in one go.', icon: Sparkles },
  ];
        ...
        {paths.map((path) => {
          ...
          return (
            <button
              ...
              className={`group relative overflow-hidden rounded-xl border p-2.5 text-left transition hover:-translate-y-0.5 ${
                isSelected
                  ? 'border-[#f3a33a]/70 bg-[#f3a33a]/[0.08] ring-1 ring-[#f3a33a]/30'
                  : 'border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.01] hover:border-[#f3a33a]/40'
              }`}
            >
