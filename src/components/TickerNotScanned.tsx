  ...
  return (
    <div className="mx-auto max-w-2xl px-3 py-10">
      <div className="terminal-panel rounded-panel p-6 text-center">
        <div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-full border border-line-strong bg-panel">
          <SearchX size={22} className="text-accent" />
        </div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-1.5 sm:grid-cols-3">
              {examples.map((t) => (
                <Link
                  ...
                  className="flex items-center gap-2 rounded-cell border border-line bg-panel px-2.5 py-1.5 transition-colors hover:border-line-strong hover:bg-line/30"
                >
                  <TickerLogo symbol={t.symbol} companyName={t.companyName} size={18} />
                  <span className="truncate font-mono text-[12px] font-semibold text-ink-title">
                    {t.symbol}
                  </span>
                </Link>
              ))}
        ...
        <Link
          href="/radar"
          className="mt-6 inline-flex min-h-[44px] items-center gap-2 rounded-cell border border-line-strong bg-panel px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-accent transition-colors hover:bg-line/30"
        >
          <Radar size={14} />
          See the full radar
        </Link>
