...
function Seg({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <span className="font-mono text-base font-semibold tabular-nums leading-none text-ink md:text-lg">
        {String(value).padStart(2, '0')}
      </span>
      <span className="mt-0.5 font-mono text-[8px] uppercase tracking-[0.12em] text-ink-dim">{label}</span>
    </div>
  );
}

function Colon() {
  return <span className="pb-3 font-mono text-sm text-line-hair">:</span>;
}
...
export function CatalystCountdown({ events = [] }: { events?: CalendarEvent[] }) {
  ...
  if (!mounted) {
    return <div className="terminal-panel h-[92px] animate-pulse rounded-panel" aria-hidden />;
  }
      ...
      <section className="terminal-panel overflow-hidden rounded-panel">
        <div className="flex items-center justify-between gap-2 px-3 py-2.5">
          <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-3">
            <Timer size={13} className="text-ink-dim" /> Big moments
          </p>
          <a href="#catalyst-radar" className="shrink-0 font-mono text-[10px] text-ink-3 transition hover:text-ink-title">
            All catalysts →
          </a>
        </div>
        <p className="px-3 pb-2.5 text-[11px] leading-snug text-ink-2">
          No upcoming moments in the curated set - it was last refreshed on{' '}
          <span className="font-mono text-accent">{CATALYSTS_CURATED_AS_OF}</span> and is due an update. An empty
  ...
  return (
    <section className="terminal-panel overflow-hidden rounded-panel">
      <div className="flex items-center justify-between gap-2 border-b border-line px-3 py-2">
        <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-3">
          <Timer size={13} className="text-accent" /> Big moments
        ...
        {featured.map((catalyst, i) => {
          ...
          return (
            <a
              key={catalyst.id}
              href={catalyst.href}
              className={`flex min-w-[228px] flex-1 flex-col gap-1.5 rounded-cell border p-2.5 transition ${
                hot ? 'border-accent-border/70 bg-accent-tint/60' : 'border-line bg-panel hover:border-line-hair'
              }`}
            >
              <div className="flex items-center gap-1.5">
                <Icon size={13} className={hot ? 'text-accent' : 'text-blue-info'} />
