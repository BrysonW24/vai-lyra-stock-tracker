...
| `/onboarding` in-app flow | Signed-in users + demo tour | Full journey for accounts AND the "Explore the demo first" entry (v0.67+): Lyra reveal -> feature primer -> setup questionnaire -> Add-to-Home-Screen beat -> success beat. The Home-Screen beat ([src/components/onboarding/AddToHomeScreenStep.tsx](src/components/onboarding/AddToHomeScreenStep.tsx)) shows a privacy-cropped iOS screenshot walkthrough (public/onboarding/a2hs-*.png) / Android steps, skips itself when already installed - functional on iPhone, where push alerts require the installed app |
