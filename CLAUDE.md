...
- `ios/` + `capacitor.config.ts` + `native/shell/` - the Capacitor 8 iOS remote shell (TestFlight beta). The WKWebView loads production (`https://lyra.vivacityai.com.au`), so every web deploy updates the native app with no resubmission; `native/shell/` holds only the offline fallback page. Shell/icon/plugin changes need a new native build - web changes never do. Lane prompts + coordination protocol: `backlog/README.md` (prompt files 1-3).
- `backlog/` - agent prompt packs and parked work items (not a live ticket system).
- `.claude/commands/` - agent playbooks (skill chains). **Every code section has an owning chain - the registry + coverage map is [`SKILL-CHAIN.md`](SKILL-CHAIN.md), enforced by `npm run check:chains` in CI; the enforcement layers around them are [`HARNESS.md`](HARNESS.md).** The chains: `/setup`, `/production-keeper`, `/feedback-loop`, `/signal-quality`, `/ai-quality`, `/notification-health`, `/onboarding-funnel`, `/onboarding-parity` (+ per-surface skills `/sync-human-onboarding`, `/sync-companion-onboarding`, `/sync-agent-onboarding`), `/data-integrity`, `/security-sweep`, `/ux-surface`, `/logs-to-genui`, `/scout-intel`, `/draft-vertical`, `/emerging-winner`. Update them when the steps they encode change.
- `COSTS.md` - fully-itemised stack costs (list prices). Update when a new paid service enters the stack.
- `DATA-ECONOMICS.md` - measured usage + free-tier runway (DB growth, call volumes, audited retention benefit horizons, tripwires). The nightly `check:data-economics` gate enforces its budgets. Re-measure via its Refresh section when a new writer joins the nightly or roughly quarterly.
...
- **Tests must not depend on the machine's timezone.** Any test exercising timezone-aware logic (`routeNotification` evaluates quiet hours in `prefs.timezone`, default `Australia/Sydney`) must pass an EXPLICIT zone AND an absolute UTC instant (`new Date('2026-06-11T13:30:00Z')`) - never a bare local clock (`new Date(2026, 5, 11, 23, 30)`). A local clock is only safe where the code under test also reads local hours with no zone (construct and read cancel out). Getting this wrong inverts the window on the UTC CI runner: it made CI red for ~9 hours of every day while passing locally in Sydney, and left the safety-critical bypass tests passing for the wrong reason (fixed 2026-07-17). Verify with `TZ=UTC npm test`.
