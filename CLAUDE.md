...
- `ios/` + `capacitor.config.ts` + `native/shell/` - the Capacitor 8 iOS remote shell (TestFlight beta). The WKWebView loads production (`https://lyra.vivacityai.com.au`), so every web deploy updates the native app with no resubmission; `native/shell/` holds only the offline fallback page. Shell/icon/plugin changes need a new native build - web changes never do. Lane prompts + coordination protocol: `backlog/ios-testflight-agent-prompt-pack.md`.
